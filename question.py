...
# Font
font = pygame.font.Font("DejaVuSans.ttf", 28)  # Reduced font size to 28 for better fit
...
def main():
    ...
    while running and current_question < len(questions):
        ...
        if current_question < len(questions):
            ...
            screen.blit(number_text, (WIDTH // 2 - number_text.get_width() // 2, 20))
            ...
            # Draw a larger box behind the question text to fit all text
            question_lines = []
            words = question_text.split()
            current_line = ""
            for word in words:
                test_line = current_line + word + " "
                if font.size(test_line)[0] <= 1000:  # Adjust width limit
            ...
            max_line_width = max(font.size(line)[0] for line in question_lines)
            question_box_width = max_line_width + 80
            question_box_height = len(question_lines) * (font.get_height() + 10) + 40
            ...
            pygame.draw.rect(screen, (240, 240, 240), question_box_rect)  # Light grey background
            pygame.draw.rect(screen, BLACK, question_box_rect, 2)        # Black border

            # Render and blit each line of the question
            for i, line in enumerate(question_lines):
                question_render = font.render(line, True, BLACK)
                screen.blit(question_render, (question_box_x + 20, question_box_y + 20 + i * (font.get_height() + 10)))

            # Draw options with adjusted spacing and size
            option_rects = []
            option_spacing = 150
            start_y = question_box_y + question_box_height + 40
